...
\begin{Def}
	...
	\]
\end{Def}
Sicuramente $\rk f \le \rk_S f$, e vale l'uguaglianza per $d = 2$. È una congettura se sono uguali, detta \emph{congettura di Comon}. Nel 2017 sembra essere stato trovato un controesempio \cite{Shitov2007}.
...
\begin{Def}[Waring rank]
	...
	\[
		\rk_S f = \min\{r \in \N \mid f = l_1^f + \dots + l_r^d, \ l_i \text{ forma lineare}\}\,.
	\]
\end{Def}
\printbibliography
\end{document}